...
- **导航** — 面包屑（分段点击 + 编辑模式）、此电脑 + 快速访问导航窗格、
  ...
  搜索聚焦（`Ctrl+E`/`Ctrl+F`/`F3`）、`Alt+Home` 回主页、`Ctrl+Shift+C` 复制为路径、
...
- **文件操作** — 新建文件夹/文件，重命名（`F2`，行内编辑），复制（带**可取消的进度对话框**），
  ...
  **撤销/重做**（`Ctrl+Z`/`Y`）
- **归档压缩 / 解压** — 右键「压缩为 ZIP」/「压缩为 7z」/「压缩为 TAR.GZ」，
  解压支持 **ZIP / 7z / RAR / TAR / TAR.GZ / TGZ**，可取消进度，解压带
  **Zip-Slip 防护**（含 `..` 的恶意条目被跳过而非中断整个解压），
  目标名自动编号（冲突安全 `(n)` 后缀）
...
- **选择** — 单击 / `Ctrl` / `Shift` / `Ctrl+A`，方向键导航（`↑↓`+`Enter`），
  图标视图下支持橡皮筋框选；启用「项目复选框」后可免按 `Ctrl` 多选
- **拖放** — 拖到文件夹 / 导航窗格 / 快速访问 / **面包屑段** 移动，`Ctrl`=复制；
  拖放高亮目标项，同名冲突弹窗 + 进度，拖入自身目录为空操作
...
- **外观** — 自定义 Mica 标题栏，浅色/深色主题（跟随系统），`F11` 全屏，
  视图→显示/隐藏（导航窗格 / 预览窗格 / 详细信息窗格 / 项目复选框 / 文件扩展名 / 隐藏的项目）、
  **紧凑视图**、导航窗格宽度可拖拽（160–480px），详细信息列宽可调、排序箭头、显示/隐藏列、分组依据
