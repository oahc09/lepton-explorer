...
![status](https://img.shields.io/badge/status-feature--complete-brightgreen) ![tests](https://img.shields.io/badge/tests-186%20passing-brightgreen) ![build](https://img.shields.io/badge/exe-windows%20x64-blue)
...
```bash
cd src-tauri && cargo test   # Rust unit tests (47)
pnpm test                    # frontend tests (139)
...
- §11 visual acceptance: a screenshot comparison was performed — WinFinder's self-captured render vs a
  real Win11 File Explorer screenshot — with **zero deviation on all measured style tokens** (accent
  `#0078D4`, command bar 40px, status bar 24px, button radius 4px, theme, font, backgrounds); see
  `docs/acceptance/section11-comparison.md`. (A pixel check on the user's own Win11 with identical content
  is the optional final confirmation.)
