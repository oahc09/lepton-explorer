...
export function ContextMenu({ entries }: { entries: Entry[] }) {
  ...
  return (
    <ul ref={menuRef} className={`context-menu${flipLeft ? ' flip-left' : ''}`} style={{ left: (box ?? pos).x, top: (box ?? pos).y }}>
      ...
      {item('压缩为 TAR.GZ', () => ops.zipTarGz(selEntries.map((e) => e.path), path), !hasSel || !realPath)}
      {(() => {
        const en = selEntries[0];
        const show = realPath && sel.length === 1 && en && !en.isDir && /\.(zip|7z|rar|tar|tar\.gz|tgz)$/i.test(en.name);
        if (!show) return null;
